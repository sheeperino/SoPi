...
void sort_intervals_horiz(unsigned char *data, char *mask, bool gay) {
  bool new_strip = true;
  uint32_t strip_col;
  int start = 0;
  for (int i = 0; i < x * y; ++i) {
...
// TODO: analyze why these intervals look "disjointed" (withotu gay)
void sort_intervals_vert(unsigned char *data, char *mask, bool gay) {
  bool new_strip = true;
  uint32_t strip_col;
